...
public class EcosConfiguration {

	public static final String REGISTRATION_ID = "minutmiljo";

	private static final Logger log = LoggerFactory.getLogger(EcosConfiguration.class);
	...
	public EcosConfiguration(final EcosProperties ecosProperties) {
		this.ecosProperties = ecosProperties;
	}
	...
	Client okHttpClient(final Truststore trustStore) {
		...
		return new OkHttpClient(new okhttp3.OkHttpClient.Builder()
			...
			.authenticator(new NTLMAuthenticator(ecosProperties.username(), ecosProperties.password()))
			.build());
	}

	@Bean
	FeignBuilderCustomizer feignBuilderCustomizer(final EcosProperties properties) {
